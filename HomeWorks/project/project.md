...
Собственно, вот - у нас есть ip-шник, который, как кластерный ресурс, можно двигать по кластеру, туда и тогда куда и когда перемещается лидер, в патрони-менеджмент кластере.
...
Бывалые люди подсказали что это, в YC, на уровне гипервизора файрволл по мак-адресам работает.

Ну, и раз такая заподлянка, от YC, с коннективити, только только haproxy сделаю, как некое изображение подключения к патрони-менеджмент бд, по единому адресу.
