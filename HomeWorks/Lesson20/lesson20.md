...
1. Партишен-прунинг - есть, при этом check-констрейнты я, умышленно, не навешивал, на партиции.
   Т.е. база ориентируется только по определениям партиций и `not null` на ключ секционирования - уже достаточно.
...
Несколько стрёмно что, прямо в доке написано - не нужно делать много партиций, более десятков тысяч: для работы с партицией пг грузит в память метаданные по ней.
Но, может поправят в следующих версиях + для небольших проектов, а может и для больших, если дадут вдумчиво отстроить ротирование данных - вполне.
...
В оракле, конечно, секционирование значительно кудрявее и богаче методов секционирования + нет никакого ограничения на кол-во партиций.
Только `partition option` - доступна только в EE-редакции, как extra-cost опция.
