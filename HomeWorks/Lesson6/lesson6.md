...
7. Тэг `twodisk_wal-segsize_128_ssd_wb64m`: условия эксперимента #5 и, отличие - выставил `synchronous_commit = off`;
...
8. Тэг `twodisk_wal-segsize_128_ssd_wb64m_scoff_fsoff`: условия предыдущего теста и, отличие - выставлен `fsync=off`, чего стесняться то. 
   Заодним посмотреть: если tps - значительно повыситься, относительно tps-а в условиях в предыдушем эксперименте, то значит фактором ограничивающим продуктовность бд, под данной нагрузкой, является не только и не столько продуктивность обработки редо-данных от транзакций, а продуктивность IO в перманентные структуры хранения данных.
