...
1. Заметка к бенчмаркингу: не надо удалять прям все wal-логи. Ибо и, понятно почему, можно словить, при попытке перезапуска субд:
   ...
   ```
   Для того чтобы новые значения конф-х пар-ров вступили в силу: перезапускать кластер не обязательно, если это явно не сказано в док-ции.
   Т.е., если не требуется, по новому значению параметра(ов), менять что то в действующем экземпляре, например размеры служебных областей памяти экземпляра - это может быть просто не возможно, в общем случае и поэтому - рестарт, оно только при рестарте сможет нарезать области какие сказано.
   По [доке](https://www.postgresql.org/docs/14/config-setting.html#CONFIG-SETTING-NAMES-VALUES) форсить перечитывание конфига можно по `pg_ctlcluster 14 main reload`
   ```
   2022-07-24 14:57:38.712 UTC [589] LOG:  received SIGHUP, reloading configuration files
   2022-07-24 14:57:38.713 UTC [589] LOG:  parameter "fsync" changed to "on"
   2022-07-24 14:57:38.713 UTC [589] LOG:  parameter "synchronous_commit" changed to "remote_apply"
   ```
   При этом, новые значения параметров появляются даже в уже открытых скл-сессиях.
   `alter system` есть, но оно меняет отдельный файл `postgresql.auto.conf`, который просто перечитывается всегда, когда перечитывается `postgresql.conf` и настройки в `postgresql.auto.conf` всегда перекрывают эти же настройки в `postgresql.conf`;
   На уровне скл-сессии задать параметр(ы) можно либо при запуске скл-сессии, либо по SET-команде, либо делая update-команду на `pg_settings`вью.
...
3. На уровне скл-сессии задать отложенный-пакетный режим обработки commit-ов, т.е., то что в пг, на уровне кластера, задаётся по `synchronous_commit = off` - можно.
   Поиск серверного процесса обслуживающего скл-сессию:
   ```sql
      [local]:5432 #postgres@postgres > set application_name to qqq1;
      [local]:5432 #postgres@postgres > select pid, usename, client_addr, wait_event, query_id, query, application_name from pg_stat_activity;
       pid  | usename  | client_addr |     wait_event      | query_id |                query                | application_name
      ------+----------+-------------+---------------------+----------+-------------------------------------+------------------
        596 | postgres |             | LogicalLauncherMain |          |                                     |
        594 |          |             | AutoVacuumMain      |          |                                     |
        780 | postgres |             | ClientRead          |          | COMMIT                              | psql
       1001 | postgres |             |                     |          | FETCH FORWARD 100 FROM _psql_cursor | qqq1
        592 |          |             | BgWriterHibernate   |          |                                     |
        591 |          |             | CheckpointerMain    |          |                                     |
        593 |          |             | WalWriterMain       |          |                                     |
      (7 rows)
   ```
...
1. Графики, с динамикой tps, от кол-ва клиентов, по сессиям:
   ...
   Это, кстати, обозначает что по дефаулту, бд делает буферизованное IO, к датафайлам, а не directIO; 
   Собственно потому и приходится делать, по умолчанию, `fsync` чтобы хоть как то защититься от превращения в тыкву данных в файловом кеше ОС-и.
   asyncIO я так понимаю - тоже нет.
   [Занятная статья](https://www.pgcon.org/2018/schedule/attachments/507_PGCon2018_Introducing_PMDK_into_PostgreSQL.pdf), на эту тему.
