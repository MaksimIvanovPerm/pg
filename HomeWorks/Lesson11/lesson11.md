...
2. кластер перезапускается, по `pg_ctlcuster`, именно - по `stop|start`, чтобы прямо всё-всё "по честному" - сбросило кеши, пересоздало кеши заданных размеров, позакрывало сессии-транзакции.
