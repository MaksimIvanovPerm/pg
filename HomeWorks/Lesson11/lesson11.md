...
2. От ОС-аккаунта `postgres`:
   ```shell
   cd ~
   wget -O pg_profile.tar.gz https://github.com/zubkov-andrei/pg_profile/releases/download/4.0/pg_profile--4.0.tar.gz
...
3. Решил варьировать такие и в таких диапазонах, пг-параметры:
   ...
   Прикинул, по максимальным значениям параметров относящихся к потреблению памяти, в сумме получается ~2Гб. 
   ...
   Видно только по изменению значений каунтеров в выводе `cat /proc/meminfo | grep -i hugepages`
   Неудобненько. 
...
Диапазон значений `[0,100]` - внутри фитнес-функции маппится в разрешённый диапазон значений для данного параметра.
Скрин с примером работы GA:
...
Т.е.: полная линейная модель (серенькая кривая, на графике выше), куда предикторами входят значения всех настроечных параметров пг, варьируемых от теста к тесту: едва объясняет обший рост значений метрики.
Т.е. да - что то, из того что я менял, в виде значений параметров пг, от теста к тесту - влияет.
Но на самом деле значимый фактор(ы), я не трогал, и то что метрика ходит вверх-вниз гораздо сильнее и не так как описывает модель - либо убогость модели.
Либо наличие ещё каких то факторов, которые в модели не утчтены (например прогретость кешей, зарезание/не зарезание ресурсов виртуалке гипервизором, что то ещё)
В качество линейной модели - углубляться не буду, на тренд намекает правильно и - достаточно, в данном случае.
...
Но и не те 60+ tps, максимальные, которые достигались, в принципе.
...
Выставил, исходя из этих размышлений, при прочих равных c предыдущим тестом, такое:
```shell
$PSQL -v ON_ERROR_STOP=1 << __EOF__ 1>/dev/null
alter system set autovacuum_max_workers=2;
...
__EOF__
```
Т.е. целенаправленно, при прочих равных пг-настройках с хорошим тестом, сделал так чтобы автовакуум - сработывал, при своём запуске, практически для любой таблицы, в которой есть более 1% мёртвых туплов.
