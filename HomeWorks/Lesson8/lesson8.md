...
5. С этих пропозиций, для технической реализации, прокодировал шелл-библиотеку `library`, с ф-циями, которыми обеспечивается выполнение теста и набор шелл-скриптов, которыми реализуется выполнение ф-ций из этой библиотеки, как операций, необходимых для выполнения теста.
   ```shell
   ...
   2. Выставляется какое то конкретное значение конф-х параметров автовакуума, выполняеся рестарт кластера.
      Этим занимается `get_test_id.sh`
   3. Выполняется тест, это вызов `runtest.sh` с передачей скрипту id-шника данного теста.
      Про себя этот скрипт, такие действия, покажу код тут, благо он короткий:
      ```shell
      runtest(){
      local v_testnum="$1"
...
9. Результаты:
   ...
   Табличная модель - с особенностями, о которых говорил выше, де факто, погоду делает одна `pgbench_accounts`, т.е. грубо говоря - остальных таблиц: как бы нет.
