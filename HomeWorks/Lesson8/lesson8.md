...
4. Вот эта часть задач в ДЗ заставила задуматься:
   ...
   Качество подбора конфигурации автовакуума - тем выше, чем больший tps и чем меньшую дисперсию tps мы получаем от субд, при выполнении бенчмарка и срабатывании автовакуума, во время бенчмарка.
   ...
   Тогда вариантов два.
   Первый вариант: то что называется "перебор по решётке", или, другое название - "метод координатного спуска".
   ...
   И выбираем то значение этой компоненты, при котором `F(X1,X2,...Xn)` - оказалось экстремальным, и далее, для этой компонеты, используем только это, выбранное значение.
   ...
   Вывод второй: надо пользоваться каким то оптимизационным алгоритмом.
   Брут-форсом эту задачу, за разумное время не решить, с достаточной точностью - чтобы было видно какие то эффекты, именно от разных настроек автовакуумирования.
...
5. С этих пропозиций, для технической реализации, прокодировал шелл-библиотеку `library` (в аттаче: [files.tar](/HomeWorks/Lesson8/files.tar)), с ф-циями, которыми обеспечивается выполнение теста и набор шелл-скриптов, которыми реализуется выполнение ф-ций из этой библиотеки, как операций, необходимых для выполнения теста.
   ...
   3. Выполняется тест, это вызов `runtest.sh` с передачей скрипту id-шника данного теста.
      ...
      Самой утилите `pgbench` сказано периодически (каждую минуту) подбивать и выводить в лог-файл данные tps, какой он был за эту минуту.
...
9. Результаты:
   ...
   Ну. Да, действительно, генетика, перебирая популяции и отсевая в следующую популяцию экземпляры с хорошими генами (те которые дают большее начение функции отбора) - очень быстро (относительно, например, метода координатного спуска) стала получать поколения видов, с такими хромосомами (где гены это - значения входного вектора `X1,X2,...,Xn`, для целевой ф-ции, она же - функция отбора в генетическом алг-ме) которые показывает значительно лучшую приспособленность, для поставленных условий отбора.
   ...
   Первые слева 5-ть значений, по размаху абсолютных величин, несколько забивают остальную динамику (я потому их и пропускаю, при подсчёте метрики теста)
   ...
   Что то может поменять вариативность в том как часто автовакуум будет обрабатывать `pgbench_accounts`
